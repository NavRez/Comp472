...
if __name__ == "__main__":
    ...
    # Ploting the distribution of the number of the instances in each class
    df = pd.DataFrame({'Positive': poSize, 'Negative': negSize}, index = ["Classes"])
    ax = df.plot.bar( rot=0)
    plot.show()
    print("negSize", negSize)
    print("poSize", poSize)
    print("negWords", negWords)
